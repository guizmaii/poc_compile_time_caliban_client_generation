...
object Resolvers {
  ...
  private val queries =
    Query(
      byName = name => ZIO.serviceWithZIO[PotatoesService](_.findByName(name)),
      byColor = color => ZIO.serviceWithZIO[PotatoesService](_.findByColor(color)),
    )

  private val mutations =
    Mutation(
      makeNewSpecies = args => ZIO.serviceWithZIO[PotatoesService](_.makeNewSpecies(args.name, args.color)),
      eradicate = name => ZIO.serviceWithZIO[PotatoesService](_.eradicate(name)),
    )
